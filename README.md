...
一个功能完整的运维监控中心，用于监控系统运行状态、网络流量、IO 运行状态和 Docker 容器运行状态。
...
**功能增强**：
- ✅ 一键部署：添加 PowerShell 版本的部署脚本，支持 Windows 环境
- ✅ 一键卸载：添加 PowerShell 版本的卸载脚本，支持 Windows 环境
- ✅ 文档更新：完善 README.md，添加不同操作系统的部署和卸载指南

### v2.0.0 (2026-02-04)

**完全重构**：
- ✅ 前端框架：从 React + TypeScript 迁移到 Vue 3 + Element Plus
- ✅ 前端目录：从 `frontend-vue/` 迁移到 `frontend/`
...
2. **代码克隆**：从 GitHub 克隆项目代码到本地（如果GitHub访问失败，会自动尝试从Gitee克隆）
3. **依赖安装**：安装前端和后端的依赖项
